...
module PGBackups
  ...
  def restore
    ...
    display_info("Backup",    "Taken from #{backup['from_name']} at #{backup['created_at']}")
  ...
  def backups
    ...
    name = args.shift
    if name
      return url(name)      if url
      return show_log(name) if log
      return delete(name)   if delete
      return backup_details(name, :long => true)
    end
    ...
    pgbackup_client.get_transfers.each { |t|
      ...
      backup_name = File.basename(t['to_url'], '.*')
      backups << [backup_name, t['created_at'], t['size'], t['from_name'], ]
    }
